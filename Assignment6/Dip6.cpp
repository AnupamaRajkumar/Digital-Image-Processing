...
void reference_convolutionForward(const Tensor &input, const Tensor &kernel, const Tensor &bias, Tensor &output)
{
	...
	for (unsigned int h = 0; h < output.getSize(0); h++) {
		for (unsigned int w = 0; w < output.getSize(1); w++) {
			for (unsigned int c_o = 0; c_o < output.getSize(2); c_o++) {
				for (unsigned int instance = 0; instance < output.getSize(3); instance++) {
					biasVal = bias(0 , 0, c_o, 0);
					for (unsigned int k_h = 0; k_h < kernel.getSize(0); k_h++) {
						for (unsigned int k_w = 0; k_w < kernel.getSize(1); k_w++) {
							for (unsigned int c_i = 0; c_i < input.getSize(2); c_i++) {
								biasVal += kernel(k_h, k_w, c_o, c_i)*input(h+k_h, w+k_w, c_i, instance);
							}
						}
					}
					output(h, w, c_o, instance) = biasVal;
				}
...
template<unsigned inputChannels,
	...
	// TODO !!!
	parallelFor(0, output.getSize(0), 1, [&](unsigned h) {
		for (unsigned int w = 0; w < output.getSize(1); w++) {
			for (unsigned int c_o = 0; c_o < output.getSize(2); c_o++) {
				simd::Scalar sum(&bias(0, 0, c_o, 0));
				simd::Vector<instances> vSum = sum;
				for (unsigned int k_h = 0; k_h < kernel.getSize(0); k_h++) {
					for (unsigned int k_w = 0; k_w < kernel.getSize(1); k_w++) {
						for (unsigned int c_i = 0; c_i < inputChannels; c_i++) {
							simd::Vector<instances> vInput(&input(h+k_h, w+k_w, c_i, 0));
							simd::Scalar k(&kernel(k_h, k_w, c_o, c_i));
							simd::Vector<instances> vKernel = k;
							vSum += vInput * vKernel;
						}
					}
				}
				vSum.store(&output(h, w, c_o, 0));
			}
		}
	});
	...
	void convolutionBackwardData_ION(const Tensor &input, const Tensor &kernel, const Tensor &bias, const Tensor &output, const Tensor &outputGrad, Tensor &inputGrad)
{
	...
	parallelFor(0, input.getSize(0), 1, [&](unsigned y) {
		for (unsigned x = 0; x < input.getSize(1); x++)
				...
				for (unsigned y_k = minY; y_k <= maxY; y_k++)
					for (unsigned x_k = minX; x_k <= maxX; x_k++)
						for (unsigned c_o = 0; c_o < outputChannels; c_o++) {
							simd::Scalar kValue(&kernel(y_k, x_k, c_o, c_i));
							simd::Vector<instances> kVector = kValue;
							simd::Vector<instances> vOutputGrad(&outputGrad(y - y_k, x - x_k, c_o, 0));
							accus += vOutputGrad * kValue;
...
*/ 
	...
	simd::Vector<8> m_input;

	for (unsigned i = 0; i < input.getTotalSize(); i = i + 8) {
		m_input.load(&input[i]);
		m_input = simd::max(m_input, zero);
		m_input.store(&m_output[i]);
	}
...
void ReLU::backward(const Tensor &input, const Tensor &outputGradients)
{
	...
	for (unsigned dim = 0; dim < input.getTotalSize(); dim = dim + 8) {
		in.load(&input[dim]);
		out.load(&outputGradients[dim]);
		simd::Vector<8> mask = in > zero;
		simd::Vector<8> vOrZero = simd::selectOrZero(mask, out);
		vOrZero.store(&m_inputGradients[dim]);
	}
}
	...
	void Adam::performStep(float stepsize)
	{
		...
		for (unsigned i = 0; i < m_parameterMomentum.size(); i++)
		{
			...
			for (unsigned i = 0; i < Tvalues.getTotalSize(); i++)
			{
				...
				float m = Tmomentum[i];
				m = m_beta1 * m + oneMinusBeta1 * g;
				Tmomentum[i] = m;

				float v = Tvariance[i];
				v = m_beta2 * v + oneMinusBeta2 * (g * g);
				Tvariance[i] = v;

				float eps = 1e-8f;
				float value = m / (std::sqrt(v) + eps);
				Tvalues[i] -= stepsize * value;
			}
		}
	}
...
float MSELoss::computeLoss(const Tensor &computedOutput, const Tensor &desiredOutput, Tensor &computedOutputGradients)
{
	...
	// The loss (scalar value, average over all pixels, channels, and instances) is returned as the function's return value.
	...
	float multiplier = 2.0f / computedOutput.getTotalSize();

	for (unsigned r = 0; r < computedOutput.getSize(0); r++)
	{	for (unsigned c = 0; c < computedOutput.getSize(1); c++)
		{	for (unsigned chan = 0; chan < computedOutput.getSize(2); chan++)
			{	for (unsigned i = 0; i < computedOutput.getSize(3); i++)
				{
					diff = computedOutput(r, c, chan, i) - desiredOutput(r + shiftY, c + shiftX, chan, i);
					average += (diff * diff);
					computedOutputGradients(r, c, chan, i) = multiplier * diff;
				}
			}
		}
	}
	// TODO (save loss and plot it).
	return (average / computedOutput.getTotalSize());
}
